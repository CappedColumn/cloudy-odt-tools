"""Tests for codt_tools.simulation (Simulation)."""
...
class TestInit:
    ...
    def test_from_nc_path(self, sim_dir):
        sim = Simulation(sim_dir / "test_sim.nc")
        assert sim.name == "test_sim"

    def test_from_stem_path(self, sim_dir):
        sim = Simulation(sim_dir / "test_sim")
        assert sim.name == "test_sim"
...
class TestConventionsGate:
    ...
    def test_supported_conventions_no_warning(self, sim_dir, recwarn):
        Simulation(sim_dir)  # fixture writes CODT_output_v1
        assert not [
...
class TestTrajectories:
    ...
    def test_has_trajectories_true(self, sim_dir_with_particles):
        sim = Simulation(sim_dir_with_particles)
...
class TestCompare:
    ...
    def test_timeseries(self, sim_dir):
        sim = Simulation(sim_dir)
        ax = Simulation.compare([sim, sim], "LWC",
    ...
    def test_profile(self, sim_dir):
        sim = Simulation(sim_dir)
        ax = Simulation.compare([sim, sim], "T",
    ...
    def test_spectrum(self, sim_dir):
        sim = Simulation(sim_dir)
        ax = Simulation.compare([sim, sim], "DSD",
                                    plot_type="spectrum",
    ...
    def test_domain_averaged_2d(self, sim_dir):
        ...
        sim = Simulation(sim_dir)
        ax = Simulation.compare([sim], "T", plot_type="timeseries")
        assert ax is not None
...
class TestNoMicrophysics:
    ...
    def test_no_dsd(self, sim_dir_no_micro):
        sim = Simulation(sim_dir_no_micro)
