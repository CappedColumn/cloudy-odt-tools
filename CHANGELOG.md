...
This is a clean break with no compatibility aliases. Code pinned to the old
names uses the `v0.8.0` tag.
...
- **Designs** — a point is a dict, and a design is a list of dicts. `cross()`
  is the only combinatorial helper. `_SWEEP_ABBREV` auto-naming deleted.
...
- **CODT 3.0.0 LEM turbulence scales** (codt_tools v0.6.0): `kolmogorov_length_scale` removed from `&TURBULENCE_LEM`. CODT now derives the smallest eddy from the grid and the diffusivities. See "CODT 3.0.0 LEM turbulence scales" below.
...
`int(psigma*N) >= n_blob` is checked per leg. Omit it and CODT makes that check
at run time instead, so existing callers keep working. `Case.write_inputs` supplies
...
Entrainment Event"). That is the only in-file hint. Otherwise distinguish the
two by `code_version`. This is attribute text only. `CODT_output_v1` is
unchanged.
...
- `Simulation.lem_scales()` returns whichever `LEM.*` attributes the file
  carries. It returns an empty dict for chamber or pre-3.0.0 runs. `info()`
  prints them.
...
back-compatible writer path, because the incompatibility lives in CODT's
namelist parser. **codt_tools 0.6.0 therefore requires a CODT 3.0.0 binary**,
and 0.5.x requires a 2.x one.
...
To influence the model's smallest eddy, change `N` or `H`, which sets `6*dz`.
...
- **Parcel input (breaking).** CODT accepts only `CODT_parcel_input_v3`.
...
- **Version strings can't answer the questions you'll ask.** Output conventions
  ...
  facts per run (registry schema v3). 0.9.0 removed all of this.
...
- **config.py**: restructured `Namelist._DEFAULTS` to match CODT's namelist groups (`do_entrainment`→`&PARAMETERS`, `pressure_limit`→`&PARCEL`, new `&ENTRAINMENT` group). Aligned all defaults to CODT code defaults. `radiation_method` default `'1d'`. Verified that the v1.0.0 binary accepts a generated namelist in both modes: the chamber run completes, and parcel+entrainment passes the namelist read.
- **simulation.py**: `load_collisions` now reads the trailing `coalesced` (i1) byte. Added the entrainment budget vars (`budget_detrain_*`, `budget_entrain_*`, `budget_n_detrained/entrained`) to `_FIELD_REGISTRY`. New `domain_volume()`, `budget_totals(cumulative=True)`, and `budget_closure()` (liquid-water mass closure vs LWC).
